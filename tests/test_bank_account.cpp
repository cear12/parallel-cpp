...
TEST_CASE(
    "withdraw succeeds and decreases the balance when funds are sufficient",
    "[bank_account]") {
  BankAccount account(100.0);
...
TEST_CASE(
    "withdraw fails and leaves the balance unchanged on insufficient funds",
    "[bank_account]") {
...
TEST_CASE("concurrent deposits from many threads never lose an update",
          "[bank_account]") {
