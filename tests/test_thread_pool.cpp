...
TEST_CASE("ThreadPool runs a submitted task and returns its result",
          "[thread_pool]") {
  ThreadPool pool(2);
...
TEST_CASE("ThreadPool actually parallelizes across multiple workers",
          "[thread_pool]") {
  ThreadPool pool(4);
  ...
  for (int i = 0; i < 4; ++i) {
    futures.push_back(pool.Enqueue([&] {
      ...
      while (now > prev_max &&
             !max_observed.compare_exchange_weak(prev_max, now)) {
      }
...
TEST_CASE("ThreadPool propagates exceptions through the returned future",
          "[thread_pool]") {
  ThreadPool pool(2);
...
TEST_CASE("ThreadPool::WaitForAll blocks until every task has completed",
          "[thread_pool]") {
  ThreadPool pool(3);
