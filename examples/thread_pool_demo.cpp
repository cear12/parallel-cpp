...
void PrintStats(const ThreadPool &pool) {
  std::cout << "  workers=" << pool.WorkerCount()
            << " active=" << pool.ActiveTaskCount()
            << " queued=" << pool.QueueSize()
            << " completed=" << pool.TotalCompletedCount() << "\n";
}
...
void BasicUsage() {
  ...
  for (int i = 1; i <= 6; ++i)
    results.push_back(pool.Enqueue(CpuIntensiveTask, i * 100));
...
void MixedWorkload() {
  ...
  for (int i = 0; i < 8; ++i) {
    io_results.push_back(pool.Enqueue(IoSimulationTask,
                                      "file_" + std::to_string(i) + ".txt",
                                      delay_dist(gen)));
  }
  for (int i = 0; i < 5; ++i)
    pool.Enqueue(CpuIntensiveTask, 50 + i * 10);
...
void ExceptionHandling() {
  ...
  for (int i = 0; i < 6; ++i)
    futures.push_back(pool.Enqueue(risky_task, i, i % 3 == 0));
